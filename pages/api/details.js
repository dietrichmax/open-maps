...
export default async function handler(req, res) {
  ...
  const geocodingResponse = await fetch(`https://nominatim.openstreetmap.org/lookup?osm_ids=${osmType}${osmId}&format=json&extratags=1&addressdetails=1&accept-language=en&polygon_geojson=1&limit=1`, {
    ...
    const wikipedia = geocodingData[0].extratags.wikipedia ? geocodingData[0].extratags.wikipedia.replace(/^.+:/, "") : geocodingData[0].extratags["brand:wikipedia"] ? geocodingData[0].extratags["brand:wikipedia"] : null
    let wikipediaData 
    let summary
    if (wikipedia) {
  ...
  res.status(200).json({
    ...
    information: {
      ...
      website: geocodingData[0].extratags.website ? geocodingData[0].extratags.website : geocodingData[0].extratags["contact:website"] ? geocodingData[0].extratags["contact:website"] : geocodingData[0].extratags.url,
