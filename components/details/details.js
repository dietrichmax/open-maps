...
import { FaRoute, FaMapMarkerAlt, FaHome, FaPhone, FaEnvelope, FaBookmark, FaAccessibleIcon, FaHamburger, FaBicycle, FaUmbrellaBeach, FaWifi, FaClock } from "react-icons/fa"
...
function Details({ result, name }) {
  ...
  async function getImage(result) {
    if (Object.keys(result).length > 0 && result.wikidata) {
      ...
      console.log("sd")
      setImage(`https://source.unsplash.com/random/?${result.type}`)
    }
  }
    ...
    return (
      <DetailsContainer>
        <DetailsWrapper onTouchStart={initialiseDrag} onMouseDown={initialiseDrag} ref={elemRef} isControlled={isControlled} height={draggableRange.bottom}>
                ...
                <InformationItem>
                  ...
                  <InformationDetails>
                    ...
                    <InformationDetailsValue>
                      {result.information.opening_hours.map((hours, i) => {
                        return <p key={i}>{hours}</p>
                      })}
                    </InformationDetailsValue>
                  </InformationDetails>
                </InformationItem>
            ...
            <InformationContainer>
              ...
              {result.details.bar ? (
                <InformationItem>
                  ...
                  <InformationDetails>
                    <InformationDetailsValue>Bar</InformationDetailsValue>
                  </InformationDetails>
                </InformationItem>
              ) : null}
          ...
          <OSMContainer>
            <p>
              <OSMLink href={`https://www.openstreetmap.org/edit?${result.osm_type}=${result.osm_id}&hashtags=maps.mxd.codes`}>Edit Node</OSMLink> on{" "}
              <OSMLink href={`https://www.openstreetmap.org/${result.osm_type}/${result.osm_id}`} style={{ marginRight: "4px" }}>
                OpenStreetMap
              </OSMLink>
              <Image
                src="https://www.openstreetmap.org/assets/osm_logo-d4979005d8a03d67bbf051b4e7e6ef1b26c6a34a5cd1b65908e2947c360ca391.svg"
                width="15"
                height="15"
                title="OpenStreetMap"
